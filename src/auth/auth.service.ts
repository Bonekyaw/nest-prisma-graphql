...
export class AuthService {
  ...
  async register(registerInput: RegisterInput) {
    const phone = registerInput.phone;
    const admin = await this.adminsService.findByPhone(phone);
    if (admin) {
  ...
  async verify(verifyInput: VerifyInput) {
    const { phone, token, otp } = verifyInput;
  ...
  async confirm(confirmInput: ConfirmInput) {
    const { phone, token, password } = confirmInput;
